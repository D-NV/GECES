...
while 1:
    ...
    if a == 1:
        ...
        while 1:
            ...
            gen += 1
            continue
        with open("bestclassifier1.pkl", "wb") as out:
            dill.dump(currentGeneration[-1], out)
    elif a == 3:
        testing_data = pd.read_csv("test_data.csv")
        testing_data = np.array(testing_data)
        with open("bestclassifier1.pkl", "rb") as inp:
        ...
        for i in range(len(tp)):
            p[i] = tp[i] / float(tp[i] + fp[i])
            r[i] = tp[i] / float(tp[i] + fn[i])

        for i in range(len(p)):
            f1_score[i] = (2 * p[i] * r[i]) / float(p[i] + r[i])
        ...
        for i in range(len(f1_score)):
            avgF1Score += f1_score[i]
        avgF1Score = avgF1Score / 8
        print("Average F1-Score of classifier is ", avgF1Score)
