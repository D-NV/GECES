...
def calcFitness(tree, sample, index):
    ...
    output = (fitness/float(samplesize))*100
    # print(output)
    tree.fitness = output
    return output
...
def crossover(t1, t2, sample, index):
    ...
    nodes_a = list(new_a.nodes)
    nodes_b = list(new_b.nodes)
    pnodes_a = []
    pnodes_b = []
    wnodes_a = []
    wnodes_b = []
...
for train_index, test_index in kf.split(dataset):
    print("Ten-fold cross validation iteration ", c)
    # print(len(train_index), "\t", train_index)
    # print(len(test_index), "\t", test_index)
    training_data, testing_data = dataset[train_index], dataset[test_index]
    training_data = np.array(training_data)
    testing_data = np.array(testing_data)
    features = ["f0", "f1", "f2", "f3", "f4", "f5", "f6", "f7", "f8", "f9"]
    ...
    while 1:
        ...
        # elif a == 2:
        currentGeneration = list(initialPopulation)
        nextGeneration = []
        count = 0
        gen = 1
        while 1:
            ...
            # for i in currentGeneration[int(0.8*maxPopulation):maxPopulation]:
            #     print(i, "  ", i.cumulativefitness)
            ...
            currentGeneration = list(nextGeneration)
        ...
        accNeutral7 = 0
        res = None
        print("\n\nStarted Testing on available test data...")

        for j in currentGeneration[-1].trees:
            for k in testing_data:
                ...
                if out > 0:
                    if currentGeneration[-1].trees.index(j) == 0:
                        res = 0
                    elif currentGeneration[-1].trees.index(j) == 1:
                        res = 1
                    elif currentGeneration[-1].trees.index(j) == 2:
                        res = 2
                    elif currentGeneration[-1].trees.index(j) == 3:
                        res = 3
                    elif currentGeneration[-1].trees.index(j) == 4:
                        res = 4
                    elif currentGeneration[-1].trees.index(j) == 5:
                        res = 5
                    elif currentGeneration[-1].trees.index(j) == 6:
                        res = 6
                    elif currentGeneration[-1].trees.index(j) == 7:
                        res = 7
                else:
                    if res is None and k[-1] != 0 and currentGeneration[-1].trees.index(j) == 0:
                        accFear0 += 1
                        # print("Fear = ", accFear0)
                    elif res is None and k[-1] != 1 and currentGeneration[-1].trees.index(j) == 1:
                        accTenderness1 += 1
                        # print("Tenderness = ", accTenderness1)
                    elif res is None and k[-1] != 2 and currentGeneration[-1].trees.index(j) == 2:
                        accAmusement2 += 1
                        # print("Amusement = ", accAmusement2)
                    elif res is None and k[-1] != 3 and currentGeneration[-1].trees.index(j) == 3:
                        accJoy3 += 1
                        # print("joy = ", accJoy3)
                    elif res is None and k[-1] != 4 and currentGeneration[-1].trees.index(j) == 4:
                        accSadness4 += 1
                        # print("sadness = ", accSadness4)
                    elif res is None and k[-1] != 5 and currentGeneration[-1].trees.index(j) == 5:
                        accDisgust5 += 1
                        # print("disgust = ", accDisgust5)
                    elif res is None and k[-1] != 6 and currentGeneration[-1].trees.index(j) == 6:
                        accAnger6 += 1
                        # print("anger = ", accAnger6)
                    elif res is None and k[-1] != 7 and currentGeneration[-1].trees.index(j) == 7:
                        accNeutral7 += 1
        ...
        #
        accFear0 = (accFear0 / (float(len(testing_data)))) * 100
        accTenderness1 = (accTenderness1 / (float(len(testing_data)))) * 100
        accAmusement2 = (accAmusement2 / (float(len(testing_data)))) * 100
        accJoy3 = (accJoy3 / (float(len(testing_data)))) * 100
        accSadness4 = (accSadness4 / (float(len(testing_data)))) * 100
        accDisgust5 = (accDisgust5 / (float(len(testing_data)))) * 100
        accAnger6 = (accAnger6 / (float(len(testing_data)))) * 100
        accNeutral7 = (accNeutral7 / (float(len(testing_data)))) * 100
        ...
        print("\n \n")
        avgaccuracy = (accFear0 + accTenderness1 + accAmusement2 + accJoy3 + accSadness4 + accDisgust5 + accAnger6 + accNeutral7) / float(8)
        print("Overall average accuracy of classifier is ", avgaccuracy, "%")
        accuracypos = (accJoy3 + accAmusement2 + accTenderness1) / float(3)
        print("\n")
        print("Average accuracy for 3 positive emotions (Joy, Amusement, Tenderness) is ", accuracypos, "%")
        accuracyneg = (accAnger6 + accDisgust5 + accSadness4 + accFear0) / float(4)
        print("\n")
